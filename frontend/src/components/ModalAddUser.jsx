...
export default function ModalAddUser({ open, onClose, onSuccess }) {
    ...
    // Reset form dan ambil daftar CCTV saat modal dibuka
    useEffect(() => {
        if (open) {
            setForm({ username: '', full_name: '', email: '', password: '', role: 'viewer', cctv_ids: [] });
            fetchCctvList();
        }
    }, [open]);

    // Ambil daftar CCTV dari API
    ...
    const handleSubmit = async (e) => {
        ...
        if (!form.password?.trim()) return showAlert('Password is required.', 'warning');
        ...
        if (payload.password.length < 6) {
             showAlert('Password min 6 characters!', 'warning');
             setSubmitting(false);
             return;
        }
        if (!payload.username) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                onSuccess(result); 
                onClose();
                showAlert(`User '${payload.full_name}' successfully added.`, 'success');
            } else {
                const err = await res.json();
                showAlert(err.error || 'Failed to add new user.', 'error');
            }
        } catch {
        ...
        <dialog open={open} className="fixed inset-0 z-50 p-6 bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6 border-b pb-2">
                <h2 className="text-2xl font-bold flex items-center gap-2 text-green-700">
                    <FaUserPlus className="w-6 h-6" /> Add New User
                </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex justify-end gap-3 pt-4 border-t">
                    ...
                    <button type="submit" disabled={submitting} className="px-5 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 transition">
