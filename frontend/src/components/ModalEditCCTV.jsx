...
import { FaTimes, FaUpload, FaCamera, FaCopy } from 'react-icons/fa';
...
export default function ModalEditCCTV({ open, onClose, onUpdate, cctvData }) {
  ...
  useEffect(() => {
    if (open && cctvData) {
      const autoUrl = cctvData.ip_address && cctvData.port && cctvData.token
        ? `rtsps://${cctvData.ip_address}:${cctvData.port}/${cctvData.token}?enableSrtp`
        : '';
  ...
  // === 2. Computed URL (fallback) ===
  const computedUrl = useMemo(() => {
    if (!form.ip || !form.port || !form.token) return '';
    return `rtsps://${form.ip}:${form.port}/${form.token}?enableSrtp`;
  }, [form.ip, form.port, form.token]);
  ...
  const updateFieldsFromUrl = (rawUrl) => {
    ...
    if (!url) {
      setForm(prev => ({ ...prev, url: computedUrl }));
      return;
    }
    ...
    try {
      ...
      const ip = u.hostname;
      const port = u.port || (u.protocol === 'rtsps:' ? '7441' : '554');
      const token = u.pathname.slice(1) + u.search;

      const ipRegex = /^(\d{1,3}\.){3}\d{1,3}$/;
      if (!ipRegex.test(ip)) throw new Error('IP tidak valid');

      setForm(prev => ({ ...prev, ip, port, token, url }));
    } catch (err) {
      setUrlError(err.message || 'URL tidak valid');
    }
  };
    ...
    if (file.type === 'application/json' || file.name.endsWith('.json')) {
      ...
      reader.onload = (e) => {
        try {
          ...
          setUploadError('JSON tidak valid');
        }
      };
      ...
      setUploadError('Hanya file JSON');
    }
  }, []);
  ...
  const loadStreamPreview = async () => {
    const urlToUse = form.url || computedUrl;
    if (!urlToUse) return alert('Isi URL dulu');
    ...
    try {
      ...
      if (!res.ok) throw new Error('Stream tidak tersedia');
  ...
  const closePolygon = () => {
    if (points.length < 3) return alert('Minimal 3 titik');
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.url && !computedUrl) return alert('URL wajib');
    ...
    try {
      ...
      setSubmitting(false);
      return alert('ROI tidak valid');
    }

    const payload = {
      name: form.name.trim(),
      location: form.location.trim(),
      ip_address: form.ip.trim(),
      port: form.port.trim(),
  ...
  return (
    <dialog open={open} className="fixed inset-0 z-50 p-6 bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold">Update CCTV</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="col-span-2">
          ...
          <div className="flex gap-2">
            ...
            <button
              type="button"
              onClick={() => navigator.clipboard.writeText(form.url || computedUrl)}
        ...
        <div className="space-y-4">
          ...
          {roiMethod === 'upload' && (
            <div className="p-4 bg-gray-50 rounded-lg border">
              ...
              {roiFile && !uploadError && <p className="mt-3 text-sm font-medium text-green-600">File: {roiFile.name}</p>}
              {uploadError && <p className="mt-3 text-sm font-medium text-red-600 bg-red-50 px-3 py-1 rounded border border-red-200">{uploadError}</p>}

              {/* ROI Existing */}
              {polygons.length > 0 && !roiFile && (
                <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                  <p className="text-sm font-medium text-green-800">ROI saat ini ({polygons.length} polygon{polygons.length > 1 ? 's' : ''})</p>
          ...
          {roiMethod === 'draw' && (
            <div className="p-4 bg-gray-50 rounded-lg border space-y-3">
              ...
              {previewError && <p className="text-sm text-red-600 bg-red-50 p-2 rounded border border-red-200">{previewError}</p>}
              ...
              {imageUrl ? (
                <div className="space-y-2">
                  <div className="flex gap-2 justify-center">
                    ...
                    <button type="button" onClick={clearDrawing} className="px-3 py-1 bg-red-600 text-white text-xs rounded">Erase</button>
                  </div>
                ...
                <div className="flex flex-col items-center justify-center h-48 border-2 border-dashed border-gray-300 rounded-lg bg-white p-8 text-center">
                  ...
                  <p className="text-sm text-gray-500">Klik tombol di atas untuk ambil gambar.</p>
                </div>
              )}
            </div>
          )}
