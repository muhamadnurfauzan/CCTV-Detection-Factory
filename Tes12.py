# Base Tes11.py integrasi dengan webserver Flask menampilkan video stream
...
import threading
import queue
import psutil  # nanti bisa dihapus
...
PERSON_MODEL_PATH = "model/yolov8n.pt"  # Opsional untuk people detection
USE_PERSON_DETECTION = False  # Flag untuk toggle people detection, ubah True kalau mau menggunakan people detection
SHOW_WINDOW = False  # Flag untuk toggle window display lokal (cv2.imshow), ubah True kalau mau
...
RESIZE_SCALE = 0.4  # Turunkan sedikit untuk kurangi beban memori
QUEUE_SIZE = 10  # Max frame di queue untuk threading
...
def process_thread(frame_queue):
    ...
    while True:
        try:
            ...
            print(f"Memory usage: {psutil.Process().memory_info().rss / 1024**2:.2f} MB")  # nanti bisa dihapus
        except queue.Empty:
            continue
        ...
        # Persiapan annotated_frame untuk visualisasi (jika SHOW_WINDOW = True atau untuk web)
        ...
        # Deteksi with error handling
        try:
            if USE_PERSON_DETECTION:
                person_results = person_model.track(frame_enhanced, conf=0.1, classes=0, persist=True)
                for person_result in person_results:
                    for person_box in person_result.boxes:
                        if person_box.id is None:
                            continue
                        px1, py1, px2, py2 = map(int, person_box.xyxy[0].cpu().numpy())
                        track_id = int(person_box.id.cpu().numpy()[0])
                        person_crop = frame[max(0, py1-20):py2+20, max(0, px1-20):px2+20]
                        ppe_results = model.track(person_crop, conf=CONFIDENCE_THRESHOLD, persist=True)
                        # Adjust koordinat kembali ke frame asli
                        for ppe_box in ppe_results[0].boxes:
                            x1, y1, x2, y2 = map(int, ppe_box.xyxy[0].cpu().numpy()) + [px1-20, py1-20, px1-20, py1-20]
                            process_detection(frame, annotated_frame_local, x1, y1, x2, y2, ppe_box.cls, ppe_box.conf, track_id, current_real_time, video_time)
            else:
        ...
        # Tampilkan window lokal jika SHOW_WINDOW = True
        if SHOW_WINDOW:
            try:
                cv2.imshow("CCTV Detection", annotated_frame_local)
...
def process_detection(frame, annotated_frame, x1, y1, x2, y2, cls_id, conf, track_id, current_real_time, video_time):
    ...
    # Gambar bounding box pada annotated_frame (untuk visualisasi jika SHOW_WINDOW = True)
...
def index():
    ...
    <body>
        ...
        <p>Access this portal at http://localhost:5000. Press Ctrl+C to stop. Last updated: 03:25 PM WIB, 10 Oct 2025.</p>
    </body>
    </html>
    """
...
def video_feed():
    def gen():
        while True:
            ...
            time.sleep(0.1)  # Rate limit 10 FPS untuk kurangi beban
