# tambah queue, threading, opsi menggunakan people detection atau menampilkan window 
...
SHOW_WINDOW = False  # Flag untuk toggle window display, ubah True kalau mau menampilkan window
...
def process_thread(frame_queue):
    ...
    while True:
        ...
        # Persiapan annotated_frame untuk visualisasi (jika SHOW_WINDOW = True)
            ...
            if USE_PERSON_DETECTION:
                ...
                for person_result in person_results:
                    for person_box in person_result.boxes:
                        ...
                        for ppe_box in ppe_results[0].boxes:
                            ...
                            process_detection(frame, annotated_frame, x1, y1, x2, y2, ppe_box.cls, ppe_box.conf, track_id, current_real_time, video_time)  # Kirim annotated_frame
            else:
                ...
                for ppe_result in ppe_results:
                    for box in ppe_result.boxes:
                        ...
                        process_detection(frame, annotated_frame, x1, y1, x2, y2, cls_id, conf, track_id, current_real_time, video_time)  # Kirim annotated_frame
        except Exception as e:
            print(f"Error deteksi: {e}")
            continue

        # Tampilkan window jika SHOW_WINDOW = True
        if SHOW_WINDOW:
            try:
